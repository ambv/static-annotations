...
time of definition and, more importantly, *where* it was defined.
Consequently, using local state in annotations is no longer possible.
As for globals, the module where the annotation was defined is the
correct context for postponed evaluation.

The ``get_type_hints()`` function automatically resolves the correct
value of ``globalns`` for functions and classes.  When running ``eval()``,
...
something like this::
    ...
    def get_class_globals(cls):
        result = {}
        result.update(vars(sys.modules[cls.__module__]))
        for child in cls.__qualname__.split('.'):
            result.update(vars(result[child]))
        return result
